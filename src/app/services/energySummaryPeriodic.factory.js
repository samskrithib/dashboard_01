  ...
  function energySummaryPeriodicFactory($log, $window, $filter, energySummaryBarColors) {
    ...
    return {
      ...
      getEnergySummarySumofLinks: function (bar) {
        ...
        actualEnergyTotal = $filter('number')(_.reduce(bar, function (memo, num) {
          return memo + num.actualEnergyConsumption;
        }, 0), 2);
        optimalEnergyTotal = $filter('number')(_.reduce(bar, function (memo, num) {
          return memo + num.optimalEnergyConsumption;
        }, 0), 2);
        onTimeOptimalEnergyTotal = $filter('number')(_.reduce(bar, function (memo, num) {
          return memo + num.onTimeOptimalEnergyConsumption;
        }, 0), 2);
        targetEnergyTotal = $filter('number')(_.reduce(bar, function (memo, num) {
      ...
      setEnergySummaryChart: function (dataColumns, graphIndicator) {
        ...
        EnergySummaryChart.ygrids([{
          value: dataColumns[3],
          text: 'Energy Target ' + dataColumns[3],
          position: 'end'
        }]);
      },
